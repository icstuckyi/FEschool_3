...
function genItem(val, complete) {
  ...
  btn.addEventListener("click", () => {
    ...
    // 할일 데이터의 업데이트 함수
    // buildTasks();
    // 없어도 동작합니다. 이유는 이벤트 버블링으로 li.addEventListener가 실행되기 때문.

    //만약 li.addEventListner 가 캡쳐링이 되거나 이벤트전파를 막는다면(stopPropagation) 버블링이 동작하지 않습니다.
...
function downloadFile() {
  ...
  // click 메서드: 클릭 가능한 요소라면 JS로 클릭을 시뮬레이션합니다.
  element.click();
